...
from test_dir.common.basic import get_config, list_outputExcelInfo, get_filePath, getNowTime


class BigAppLogin(unittest.TestCase):

    #### 引入测试数据 DDT 驱动
    jsonData = pd.read_excel(get_filePath("test_data/mmc_bigapp/bigapp_testdata.xls"), sheet_name="用户登录")
    # print(jsonData)
    ...
    def test_userLogin(self, testData, assertStatusCode, assertData):
        """
        用户登录接口测试
        """
        self.log.info('---------- 测试开始 ----------')
        ...
        resp = self.se.post(url=self.userLoginUrl, headers=self.headers, data=testData)
